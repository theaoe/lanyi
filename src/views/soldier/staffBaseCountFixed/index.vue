<template>
  <div class="app-container">
    <el-form
      ...
      label-width="68px"
    >
      <el-form-item label="方案" prop="moduleId">
        <el-select
...
import {
  ...
  updateStaffBase,
  exportStaffBase,
  listStaffBaseForFixed,
  exportForFixed,
} from "@/api/soldier/staffBase";
import { listModule, getModule } from "@/api/soldier/module";
export default {
  name: "StaffBase",
  components: {},
  data() {
  ...
  methods: {
    ...
    getListForFixed() {
      this.loading = true;
      listStaffBaseForFixed(this.queryParams)
        .then((res) => {
          this.staffBaseList = res.rows;
    ...
    handleExport() {
      ...
      this.$confirm("是否确认导出人员基础信息数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return exportForFixed(queryParams);
        })
        .then((response) => {
